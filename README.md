...
3. **편리한 로컬 대시보드 UI**
   ...
   - **옵시디언 연동**: 변환이 끝난 뒤 버튼 클릭 하나로 변환된 보관소 폴더를 Finder 창으로 직접 엽니다.
...
- macOS 사용자분들은 터미널에서 `python3`을 입력하시거나 실행기를 실행할 때 Xcode 명령줄 도구 설치 팝업이 나타날 수 있으니 승인하여 설치해 주시면 됩니다.
...
### 2. 더블클릭 실행
폴더 내에 있는 **`run.command`** 파일을 마우스로 **더블클릭**합니다.
...
>
> 이후 검은색 터미널 창이 자동으로 열리며, 변환에 필요한 파이썬 모듈(`Flask`, `pyhwp`, `olefile` 등)을 최초 1회 자동으로 다운로드하고 **로컬 서버(http://localhost:5001)가 즉시 열립니다.**
...
### 4. 종료 방법
실행 중인 검은색 터미널 창을 닫거나, 터미널 창에서 **`Ctrl + C`**를 누르면 안전하게 백엔드 서버가 종료됩니다.
...
├── run.command         # 맥 OS 원클릭 자동 실행 스크립트
